...
import getGcpChannelFactory, { GcpChannelFactoryInterface } from './gcp_channel_factory';
...
export = function(grpc: GrpcModule) {

  const GcpChannelFactory = getGcpChannelFactory(grpc);
  /**
    ...
    const postProcessInterceptor = (
      ...
      const requester = {
        start: (
          ...
          const newListener = {
            onReceiveMetadata: (metadata: grpcType.Metadata, next: Function) => {
              next(metadata);
            },
            // tslint:disable-next-line:no-any protobuf message
            ...
            onReceiveStatus: (status: grpcType.StatusObject, next: Function) => {
